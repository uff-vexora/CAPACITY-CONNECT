...
const DEFAULT_UPCOMING_SESSIONS = [
  {
    id: "sess-1",
...
export function LiveSessions({ setPage }) {
  const [sessions, setSessions] = useState(() => {
    try {
      const saved = localStorage.getItem("capacity_live_sessions");
  ...
  const handleCreateSession = (e) => {
    ...
    setSessions(updated);
    try {
      localStorage.setItem("capacity_live_sessions", JSON.stringify(updated));
    } catch (err) {
      console.error(err);
    }
